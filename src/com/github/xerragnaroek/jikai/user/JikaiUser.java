...
import com.github.xerragnaroek.jikai.util.prop.*;
...
public class JikaiUser {
    ...
    private long id;
    private final IntegerProperty aniId = new IntegerProperty(0);
    private final Property<TitleLanguage> titleLanguage = new Property<>();
    private final SubscriptionSet subscribedAnime = new SubscriptionSet();
    private final BooleanProperty sendDailyUpdate = new BooleanProperty();
    private final BooleanProperty sendWeeklySchedule = new BooleanProperty();
    private final BooleanProperty sendNextEpMessage = new BooleanProperty();
    private final SetProperty<Integer> notifBeforeRelease = new SetProperty<>();
    private final BooleanProperty showAdult = new BooleanProperty();

    // set of users that are linked to this user
    private final SetProperty<Long> linkedUsers = new SetProperty<>();
    private final MapProperty<Integer, String> customTitles = new MapProperty<>();
    private final SetProperty<Integer> hiddenAnime = new SetProperty<>();

    // set of users this user is linked to
    @JsonIgnore
    private final Set<Long> linkedToUsers = new TreeSet<Long>();
    @JsonIgnore
    private final Property<ZoneId> zone = new Property<>();
    private final Property<String> locale = new Property<>("en");
    @JsonIgnore
    ...
    public UserSnowflake getSnowflake() {
            ...
            if (!Core.INITIAL_LOAD.get()) {
                ...
                if (subbed) {
                    if (ju.isShownAdult() || !AnimeDB.getAnime(id).isAdult()) {
                        ju.subscribeLinked(id, ju.getLocale().getStringFormatted("ju_sub_add_cause_linked", List.of("name"), getUser().getName()));
                    }
                }
            }
        }
    ...
    public void addPreReleaseNotificaionStep(int seconds) {
        if (notifBeforeRelease.add(seconds) && !loading) {
            ...
            String msg = seconds == 0 ? loc.getString("ju_notify_release_true") : loc.getStringFormatted("ju_step_add", List.of("time"), BotUtils.formatSeconds(seconds, loc));
            sendPM(msg);
        }
    }
    ...
    public void removePreReleaseNotificationStep(int seconds) {
        if (notifBeforeRelease.remove(seconds) && !loading) {
            JikaiLocale loc = getLocale();
            String msg = seconds == 0 ? loc.getString("ju_notify_release_false") : loc.getStringFormatted("ju_step_rem", List.of("time"), BotUtils.formatSeconds(seconds, loc));
            sendPM(msg);
        }
    }
    ...
    public CompletableFuture<Boolean> sendPM(MessageCreateData message) {
        return BotUtils.sendPMChecked(getUser(), message);
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof JikaiUser ju) {
            return ju.getId() == id;
        } else {
            return false;
        }
