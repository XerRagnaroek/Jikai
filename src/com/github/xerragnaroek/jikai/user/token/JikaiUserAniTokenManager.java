...
public class JikaiUserAniTokenManager {
    ...
    private static void invalidToken(int aniId) {
        ...
        if (ju != null) {
            ...
            ju.sendPM(BotUtils.titledEmbed(loc.getString("ju_eb_ani_auth_rev_title"), loc.getStringFormatted("ju_eb_ani_auth_rev_auto", List.of("link"), getOAuthUrl()))).thenAccept(b -> log.debug("Sent invalid token embed to user {} {}", aniId, b));
        }
    }
        ...
        Core.EXEC.execute(() -> {
            try {
                ...
                if (e instanceof NoSuchFileException) {
                    log.error("Excepetion while watching codes! No such file! Not restarting watch thread.", e);
                    BotUtils.sendThrowableToDev("Excepetion while watching codes! No such file! Not restarting watch thread.", e);
                } else {
