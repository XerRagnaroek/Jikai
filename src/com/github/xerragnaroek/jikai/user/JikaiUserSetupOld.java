...
public class JikaiUserSetupOld extends ListenerAdapter {
    private final Logger log;
    private final JikaiUser ju;
    private int stage = -1;
    private final Jikai j;

    public JikaiUserSetupOld(JikaiUser ju, Jikai j) {
        this.ju = ju;
        this.j = j;
        log = LoggerFactory.getLogger(JikaiUserSetupOld.class + "#" + ju.getId());
        Core.JDA.addEventListener(this);
    }

    public void startSetup() {
        log.debug("Running setup for new user");
        CompletableFuture<Boolean> cf = ju.sendPMFormat(ju.getLocale().getStringFormatted("setup_greetings", List.of("name"), ju.getUser().getName()));
        cf.whenComplete((b, e) -> {
            if (b) {
                nextStage();
            } else {
    ...
    private boolean language(String input) throws InterruptedException, ExecutionException {
        ...
        if (input == null) {
            ...
            ju.sendPM(JikaiLocaleManager.getEN().getStringFormatted("setup_lang", List.of("langs"), jlm.getLocales().stream().map(jl -> String.format("%s[%s]", jl.getLanguageName(), jl.getIdentifier())).collect(Collectors.joining(", "))));
        } else {
            log.debug("Stage Language: Computing input");
            if (jlm.hasLocale(input)) {
                JikaiLocale loc = jlm.getLocale(input);
                ju.setLocale(loc);
                ju.sendPM(loc.getStringFormatted("setup_lang_success", List.of("lang"), loc.getLanguageName())).get();
                log.debug("Stage Language: Lang set to {}", loc.getIdentifier());
            } else {
                log.debug("Stage Language: Unrecognized input");
                ju.sendPM(JikaiLocaleManager.getEN().getStringFormatted("setup_lang_fail", List.of("input"), input)).get();
                return false;
            }
        }
        return true;
    ...
    private boolean timeZone(String input) throws InterruptedException, ExecutionException {
        ...
        if (input == null) {
            ...
            ju.sendPM(loc.getStringFormatted("setup_timezone", List.of("servertz"), j.getJikaiData().getTimeZone().getId()));
        } else {
            try {
                ...
                ju.setTimeZone(z);
                ju.sendPM(loc.getStringFormatted("setup_timezone_success", List.of("zone"), z.getId())).get();
                log.debug("Stage TimeZone: TZ set to {}", z);
            } catch (DateTimeException e) {
                ju.sendPM(loc.getStringFormatted("setup_timezone_fail", List.of("input"), input)).get();
    ...
    private boolean linkAniAccount(String input) throws InterruptedException, ExecutionException {
        ...
        if (input == null) {
            ...
            if (input.equals("no") || input.equals("n")) {
                ...
                AniLinker.linkAniAccount(ju, input).getFuture().whenComplete((b, e) -> {
                    if (b) {
                        setupComplete();
                    }
                });
    ...
    public static void runSetup(JikaiUser ju, Jikai j) {
        new JikaiUserSetupOld(ju, j).startSetup();
    }
}
