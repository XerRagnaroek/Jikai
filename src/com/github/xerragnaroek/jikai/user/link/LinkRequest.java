...
public class LinkRequest extends ListenerAdapter {
    ...
    private final JikaiUser initiator;
    private final JikaiUser target;
    private final int dir;
    private Message msg;
    private ScheduledFuture<?> future;
    private final String optMsg;
    ...
    private void unidi() {
        ...
        initiator.sendPM(BotUtils.embedBuilder().setTitle(loc.getString("ju_link_req_eb_suc_title")).setDescription(loc.getStringFormatted("ju_link_req_unidi_suc", List.of("name"), target.getUser().getName())).setThumbnail(target.getUser().getEffectiveAvatarUrl()).build());
        log.debug("linked");
    }
    ...
    private void bidi() {
        ...
        EmbedBuilder eb = BotUtils.embedBuilder().setTitle(loc.getStringFormatted("ju_link_req_tgt_eb_title", List.of("name"), initName)).setDescription(loc.getStringFormatted("ju_link_req_tgt_msg", Arrays.asList("name", "time"), initName, BotUtils.formatMinutes(bidiRequestDuration, target.getLocale()))).setThumbnail(initiator.getUser().getEffectiveAvatarUrl());
        if (optMsg != null) {
        ...
        BotUtils.sendPM(target.getUser(), eb.build()).thenAccept(m -> {
            msg = m;
            m.addReaction(Emoji.fromUnicode(checkMark)).and(m.addReaction(Emoji.fromUnicode(crossMark))).submit().thenAccept(v -> {
                log.debug("reactions added");
                m.pin().submit().thenAccept(vo -> log.debug("request pinned"));
                initiator.sendPM(BotUtils.embedBuilder().setDescription(initiator.getLocale().getStringFormatted("ju_link_req_init_msg", List.of("name"), target.getUser().getName())).setThumbnail(target.getUser().getEffectiveAvatarUrl()).build());
    ...
    public void onMessageReactionAdd(MessageReactionAddEvent event) {
        if (event.getMessageIdLong() == msg.getIdLong() && event.getUserIdLong() == target.getId()) {
            switch (event.getEmoji().asUnicode().getAsCodepoints()) {
    ...
    private void reqAccepted() {
        ...
        EmbedBuilder eb = BotUtils.embedBuilder();
        JikaiLocale loc = initiator.getLocale();
        eb.setTitle(loc.getString("ju_link_req_eb_suc_title")).setDescription(loc.getStringFormatted("ju_link_req_bidi_suc", List.of("name"), target.getUser().getName())).setThumbnail(target.getUser().getEffectiveAvatarUrl());
        initiator.sendPM(eb.build());
        eb = BotUtils.embedBuilder();
        loc = target.getLocale();
        eb.setTitle(loc.getString("ju_link_req_eb_suc_title")).setDescription(loc.getStringFormatted("ju_link_req_bidi_suc", List.of("name"), initiator.getUser().getName())).setThumbnail(initiator.getUser().getEffectiveAvatarUrl());
    ...
    private void reqDeclined() {
        ...
        msg.unpin().submit().thenAccept(v -> log.debug("message unpinned"));
        EmbedBuilder eb = BotUtils.embedBuilder();
        eb.setTitle(initiator.getLocale().getStringFormatted("ju_link_req_bidi_init_dec", List.of("name"), target.getUser().getName())).setThumbnail(target.getUser().getEffectiveAvatarUrl());
        initiator.sendPM(eb.build());
        eb = BotUtils.embedBuilder();
        eb.setTitle(target.getLocale().getStringFormatted("ju_link_req_bidi_tgt_dec", List.of("name"), initiator.getUser().getName())).setThumbnail(initiator.getUser().getEffectiveAvatarUrl());
