...
public class Core {
    ...
    public static void main(String[] args) throws LoginException, InterruptedException, ExecutionException, ClassNotFoundException, IOException {
        ...
        handleArgs(args);
        JDABuilder builder = JDABuilder.create(token, GatewayIntent.DIRECT_MESSAGES, GatewayIntent.MESSAGE_CONTENT, GatewayIntent.DIRECT_MESSAGE_REACTIONS, GatewayIntent.GUILD_MESSAGE_REACTIONS, GatewayIntent.GUILD_MESSAGES, GatewayIntent.GUILD_MEMBERS);
        builder.disableCache(Arrays.asList(CacheFlag.VOICE_STATE, CacheFlag.EMOJI));
        ...
        JDA = builder.build();
        JDA.awaitReady();
        System.setErr(new PrintStream(WriterOutputStream.builder().setWriter(new PMWriter(Core.DEV_IDS.get(0))).setCharset(Charset.defaultCharset()).setBufferSize(1024).setWriteImmediately(true).get()));
        init(args);
        sendOnlineMsg(start);
    ...
    private static void init(String[] args) {
        ...
        JM.forEach(Jikai::validateMemberRoles);
        ...
        JM.forEach(j -> {
            ...
            try {
                if (j.hasInfoChannelSet()) {
                    j.getInfoChannel().sendMessage(loc.getStringFormatted("g_online_msg", List.of("time"), BotUtils.formatMillis(millis, loc))).submit();
                }
            } catch (Exception e) {
                ERROR_LOG.error("", e);
            }
