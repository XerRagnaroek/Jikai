...
public class JikaiSetup extends ListenerAdapter {
    ...
    private void startSetup() {
        ...
        try {
            ...
            if (setTc != null) {
                setTc.sendMessage(j.getLocale().getStringFormatted("g_setup_error", List.of("error"), e.getMessage())).queue();
            }
        }
        MDC.remove("id");
    }
    ...
    private void makeSetupChannel() throws InterruptedException, ExecutionException {
        ...
        setTc = g.createTextChannel("jikai_setup").addPermissionOverride(g.getPublicRole(), Arrays.asList(Permission.EMPTY_PERMISSIONS), List.of(Permission.VIEW_CHANNEL)).addPermissionOverride(owner, Arrays.asList(Permission.VIEW_CHANNEL, Permission.MESSAGE_MANAGE), Arrays.asList(Permission.EMPTY_PERMISSIONS)).addPermissionOverride(g.getSelfMember(), Arrays.asList(Permission.VIEW_CHANNEL, Permission.MESSAGE_MANAGE), Arrays.asList(Permission.EMPTY_PERMISSIONS)).submit().get();
    ...
    private void greetGuildOwner() throws InterruptedException, ExecutionException {
        ...
        setTc.sendMessage(j.getLocale().getStringFormatted("g_setup_greet", List.of("owner"), owner.getAsMention())).submit().get();
        MDC.remove("id");
    }
    ...
    private boolean language(String input) throws InterruptedException, ExecutionException {
        ...
        if (input == null) {
            ...
            makeAndSendEmbed(jl.getString("g_setup_lang_title"), jl.getStringFormatted("g_setup_lang_desc", List.of("langs"), JikaiLocaleManager.getInstance().getLocaleIdentifiers().toString()));
    ...
    private void finishSetup() throws InterruptedException, ExecutionException {
        ...
        makeAndSendEmbed(loc.getString("g_setup_finished_title"), loc.getStringFormatted("g_setup_finished_desc", List.of("pre"), j.getJikaiData().getPrefix()));
        JikaiData jd = j.getJikaiData();
