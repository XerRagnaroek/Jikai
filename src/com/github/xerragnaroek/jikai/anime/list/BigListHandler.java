...
public class BigListHandler {
    ...
    private static final Map<Long, Map<String, Map<Integer, Long>>> loadedData = new HashMap<>();
    private final Jikai j;
    private final BidiMap<Integer, Long> messages = new TreeBidiMap<>();
    private Predicate<Anime> filter = a -> true;
    private long tc;
    private final String identifier;
    private final Logger log;
    private final Queue<Anime> missingMessage = new LinkedList<>();
    ...
    private MessageCreateData makeMessage(Anime a, boolean addBtn) {
        ...
        MessageCreateBuilder bob = MessageCreateBuilder.from(MessageCreateData.fromEmbeds(damb.build()));
        if (addBtn) {
            bob.addComponents(ActionRow.of(Button.secondary(GeneralSubHandler.IDENTIFIER + ":" + a.getId(), Emoji.fromUnicode("U+1F514"))));
        }
        return bob.build();
    }
    ...
    public CompletableFuture<?> sendList() {
        try {
            ...
            return BotUtils.clearChannel(tc).thenAccept(vo -> {
                ...
                CompletableFuture.allOf(cfs.toArray(new CompletableFuture<?>[0])).whenComplete((v, t) -> {
                    if (t != null) {
    ...
    private CompletableFuture<Message> sendMessage(Anime a, TextChannel tc) {
        MessageCreateData m = makeMessage(a, true);
            ...
            try {
                ...
                if (tc != null) {
                    tc.getIterableHistory().submit().thenAccept(l -> {
                        ...
                        List<Long> ids = l.stream().map(Message::getIdLong).toList();
                        List<Long> loadedIds = messages.values().stream().toList();
                        loadedIds.stream().filter(id -> !ids.contains(id)).peek(id -> log.debug("Removing invalid mapping for {}", id)).forEach(messages::removeValue);

                        // validate via anime
                        log.debug("Validating via anime");
                        List<Integer> curAnime = AnimeDB.getAiringOrUpcomingAnime().stream().filter(filter).sorted(Anime.SORT_BY_RELEASE_DATE).map(Anime::getId).toList();
                        List<Integer> mappedAnime = messages.keySet().stream().toList();
    ...
    private void handleInfoChange(List<Anime> list, TextChannel tc) {
        ...
        list.forEach(a -> {
            if (messages.containsKey(a.getId())) {
                tc.editMessageById(messages.get(a.getId()), MessageEditData.fromCreateData(makeMessage(a, true))).queue(v -> log.debug("Successfully edited message {} for {},{}", v.getIdLong(), a.getId(), a.getTitleRomaji()));
            } else {
    ...
    private void handleNew(List<Anime> list, TextChannel tc) {
        ...
        log.debug("Handling {} new animes and {} missing a message", list.size(), newAnime.size());
