...
public class AnimeListHandler {
    ...
    private static final Map<Long, Map<TitleLanguage, Map<Long, List<Integer>>>> initMap = new ConcurrentHashMap<>();

    private final Map<Long, List<Integer>> msgAniMap = new ConcurrentHashMap<>();
    private final Map<Integer, Long> aniMsgMap = new ConcurrentHashMap<>();
    ...
    private List<Pair<MessageCreateData, List<Anime>>> makeMessage(String title, List<Anime> anime) {
        anime.sort(sortingBy);
        List<List<Anime>> partitioned = BotUtils.partitionCollection(anime, 25);
        List<Pair<MessageCreateData, List<Anime>>> messages = new LinkedList<>();
