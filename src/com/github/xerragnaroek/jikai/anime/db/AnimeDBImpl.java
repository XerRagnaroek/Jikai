...
class AnimeDBImpl {
	...
	private final Map<Integer, Anime> separatelyLoaded = new ConcurrentHashMap<>();
	private final Map<Integer, BufferedImage> coverImages = new ConcurrentHashMap<>();
	private final Logger log = LoggerFactory.getLogger(AnimeDBImpl.class);
	private final AtomicBoolean loading = new AtomicBoolean(true);
	private final AtomicBoolean initialized = new AtomicBoolean(false);
	private final JASA jasa = new JASA();
	private final Set<Consumer<AnimeUpdate>> updateCon = Collections.synchronizedSet(new HashSet<>());
	...
	private void loadImages() {
		ForkJoinPool limit = new ForkJoinPool(Runtime.getRuntime().availableProcessors());
		try {
			limit.submit(() -> anime.values().stream().parallel().filter(a -> !coverImages.containsKey(a.getId())).forEach(a -> {
			...
			log.error("Oppsie! Error loading the images :(", e);
		}
		limit.shutdown();
	}
