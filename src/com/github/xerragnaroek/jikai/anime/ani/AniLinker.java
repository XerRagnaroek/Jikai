...
class UserChecker extends ListenerAdapter {
    ...
    private final Map<Long, User> msgIdUser = new HashMap<>();
    private final AtomicLong sureMsgId = new AtomicLong(0);
    private final Queue<Long> sentMsgs = new LinkedList<>();
    ...
    void start() {
        ...
        BotUtils.sendPM(ju.getUser(), ju.getLocale().getStringFormatted("ju_link_ani_list", List.of("amount"), user.size())).get(0).thenAccept(m -> sentMsgs.add(m.getIdLong()));
    ...
    private CompletableFuture<Message> sendUserEmbed(User u, String... reactions) {
        ...
        return BotUtils.sendPM(ju.getUser(), makeUserEmbed(u)).thenApply(m -> {
            for (String react : reactions) {
                m.addReaction(Emoji.fromUnicode(react)).submit();
            }
    ...
    public void onMessageReactionAdd(MessageReactionAddEvent event) {
        if (event.isFromType(ChannelType.PRIVATE) && event.getUserIdLong() != Core.JDA.getSelfUser().getIdLong()) {
            long msgId = event.getMessageIdLong();
            if (msgIdUser.containsKey(msgId)) {
                String ucp = event.getEmoji().asUnicode().getAsCodepoints();
    ...
    private void correctUser(User u) {
        ...
        ju.sendPM(BotUtils.makeSimpleEmbed(ju.getLocale().getStringFormatted("ju_link_ani_done", List.of("authLink"), JikaiUserAniTokenManager.getOAuthUrl())));
        stop(false);
    }
