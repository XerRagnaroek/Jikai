		...
		if (cps.isEmpty()) {
			if (stage == 0) {
				if (!pStage.externalAdvance()) {
					ra = m.addReaction(FORWARD_CP);
				}
			} else {
				if (!pStage.externalRetreat()) {
					ra = m.addReaction(BACKWARD_CP);
				}
			}
		} else {
			if (stage > 0 && !pStage.externalRetreat()) {
				...
				ra = m.addReaction(cps.get(c++));
			}
			for (; c < cps.size(); c++) {
				ra = ra.and(m.addReaction(cps.get(c)));
			}
		}
		if (stage + 1 < stages.size() && !pStage.externalAdvance()) {
			ra = ra.and(m.addReaction(FORWARD_CP));
		}
		return ra.submit();
	}
...
record PaginationStage(MessageEmbed embed, List<String> reactionCodePoints, Optional<Consumer<String>> reactionAddHandler, Optional<Consumer<String>> reactionRemHandler, boolean externalAdvance, boolean externalRetreat, IntConsumer onStageChange) {}