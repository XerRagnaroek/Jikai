...
public class HelpCommand implements GuildCommand, JUCommand {
    ...
    private void help(User u, Jikai j) {
        ...
        BotUtils.splitForMessages(loc.getStringFormatted("com_help_eb_server_desc", List.of("sercoms"), serComs)).forEach(msg -> {
            eb.setDescription(msg);
            BotUtils.sendPMChecked(u, eb.build());
        });
        eb.setTitle(loc.getString("com_help_eb_pm_title"));
        BotUtils.splitForMessages(loc.getStringFormatted("com_help_eb_pm_desc", List.of("pmcoms"), pmComs)).forEach(msg -> {
            eb.setDescription(msg);
            BotUtils.sendPMChecked(u, eb.build());
        });
    }
