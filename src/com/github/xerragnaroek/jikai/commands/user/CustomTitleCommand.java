			...
			if (a != null) {
				if (titleStartsAt > arguments.length) {
					if (ju.hasCustomTitle(a.getId())) {
						...
						ju.sendPM(loc.getStringFormatted("com_ju_custom_title_no", Arrays.asList("anime"), a.getTitle(ju.getTitleLanguage())));
					}
				} else {
					String title = "";
				...
				ju.sendPM(loc.getStringFormatted("com_ju_custom_title_invalid", Arrays.asList("input"), String.join(" ", arguments)));
			}
		}
	}
