...
public class LinkUserCommand implements JUCommand {
    ...
    public void executeCommand(JikaiUser ju, String[] arguments) {
        ...
        switch (res) {
            case 0 ->
                    ju.sendPM(BotUtils.embedBuilder().setTitle(loc.getStringFormatted("com_ju_link_fail2", List.of("name"), tgt.getUser().getName())).build());
            case 2 ->
                    ju.sendPM(BotUtils.embedBuilder().setDescription(loc.getStringFormatted("ju_link_req_dupe", List.of("name"), u.getName())).setThumbnail(u.getEffectiveAvatarUrl()).build());
            case 3 ->
                    ju.sendPM(BotUtils.embedBuilder().setDescription(loc.getStringFormatted("ju_link_req_dupe_tgt", List.of("name"), u.getName())).setThumbnail(u.getEffectiveAvatarUrl()).build());
