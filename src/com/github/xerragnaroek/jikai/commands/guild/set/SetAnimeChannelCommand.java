    ...
    public void executeCommand(MessageReceivedEvent event, String[] arguments) {
        ...
        if (arguments.length >= 1) {
            ...
            if (!tcs.isEmpty()) {
                ...
                textC.sendMessage(j.getLocale().getStringFormatted("com_g_set_ani_fail", List.of("channel"), arguments[0])).queue();
                return;
            }
        }
        j.getJikaiData().setAnimeChannelId(textC.getIdLong());
        try {
            j.getInfoChannel().sendMessage(j.getLocale().getStringFormatted("com_g_set_ani_success", List.of("channel"), textC.getAsMention())).queue();
        } catch (Exception e) {
        }
    }
