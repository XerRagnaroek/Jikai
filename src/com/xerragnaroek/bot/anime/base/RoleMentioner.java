...
public class RoleMentioner {
	...
	public static void mentionUpdate(Guild g, Anime anime, ReleaseTime time) {
		TextChannel tc = g.getTextChannelById(GuildDataManager.getDataForGuild(g).get(GuildDataKey.ANIME_CHANNEL));
		if (tc != null) {
			...
			tc.sendMessage(makeMessage(g, anime, time)).queue(v -> log.info("Successfully sent message to guild {}, textchannel {}", g.getId(), tc.getName()), e -> log.error("Failed sending message", e));
		}
	}

	private static MessageEmbed makeMessage(Guild g, Anime a, ReleaseTime time) {
		EmbedBuilder eb = new EmbedBuilder();
		eb.setImage(a.imageURL).addField(String.format("**%s**\nreleases in **%d** days, **%d** hours and **%d** %s\n%2$02d:%3$02d:%4$02d", a.title, time.days(), time.hours(), time.mins(), (time.mins() == 1) ? "minute" : "minutes"), "", false);
